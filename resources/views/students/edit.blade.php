        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3">
                <div class="d-flex align-items-center justify-content-between">
                    <h5 class="fw-bold mb-0 text-dark">Edit Student: {{ $student->user->name }}</h5>
                ...
                <form action="{{ route('admin.students.update', $student) }}" method="POST">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="student_code" name="student_code" class="form-control @error('student_code') is-invalid @enderror" value="{{ old('student_code', $student->student_code) }}" required>
                            @error('student_code')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select id="level" name="level" class="form-select @error('level') is-invalid @enderror" required>
                                <option value="1" {{ old('level', $student->level) == 1 ? 'selected' : '' }}>Level 1 (Freshman)</option>
                                <option value="2" {{ old('level', $student->level) == 2 ? 'selected' : '' }}>Level 2 (Sophomore)</option>
                                <option value="3" {{ old('level', $student->level) == 3 ? 'selected' : '' }}>Level 3 (Junior)</option>
                                <option value="4" {{ old('level', $student->level) == 4 ? 'selected' : '' }}>Level 4 (Senior)</option>
                            </select>
                            @error('level')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
